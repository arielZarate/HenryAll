...
let x = document.querySelector("#createdBy");

x.innerHTML = "Aplicación creada por Ariel";
...
function buildToDo(todo, index) {
    ...
    // toDoShell = document.getElementsByClassName("toDoShell"); //2 ??? mall
    ...
    //toDoText.id = index; //5  donde tiene el id el objeto
    //ESTABA MAL DEBO USAR EL SET ATRIBUTE
    ...
    //este evento es parte del ultimo ejercicio
...
function buildToDos(toDos) {
    ...
    //let array = new ToDo(); MALLLL

    let array = [];

    //array=toDos.map((todo,index) => array.push(e));   malll todo mal

    array = toDos.map((todo, index) => buildToDo(todo, index));

    //por cada iteraccion crea un nuevo objeto buildTodo pasandole valor y clave
    return array;
}
...
function displayToDos() {
    ...
    let toDoContainer = document.getElementById("toDoContainer"); //1
    toDoContainer.innerHTML = ""; //2

    let resultado = buildToDos(toDoItems); //3

    //4
    resultado.forEach((element) => {
        //toDoContainer.append(element);  // ESTA MALL ES
        toDoContainer.appendChild(element); // ESTA MALL ES
    });
}
...
function addToDo() {
    // Tu código acá:
    let value = document.getElementById("toDoInput").value; //me falto agregar el values ojooo!!
    let todo = new ToDo(value); //1
...
let boton = document.getElementById("addButton"); //1
boton.addEventListener("click", addToDo); //2
