...
Node.prototype.maxBalanceFactor = function(m) {
    ...
    if (this.right && this.left)
        return max(this.right.maxBalanceFactor(m), this.left.maxBalanceFactor(m));
    ...
    if (thisbf < 0 && this.maxBalanceFactor() >= abs(thisbf)) {
        ...
        if (this.left)
            if (!this.left.isBalanced())
                return this.left.balance(this, arbol, "right");
            //este arbol esta desbalanceado
        if (this == arbol) {
            return this.rotate();
        } else {
            anterior[from] = this.rotate();
        }
    }
    return arbol;
};
